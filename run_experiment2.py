...
for i, num_vertex in enumerate([10, 50, 100, 150]): # (2.in,10), (3.in,50), (4.in,100), (5.in.150)
    for N in range(1,5):
        # run experiment
        cmd = "srun -p batch --overcommit -N{0} -n{1} ./APSP_MPI_sync testcase/{2}.in out/{2}.out {1} > log".format(N, num_vertex, i+2)
        print('#'+cmd)
        os.system(cmd)
        ...
        with open('log', 'r') as f:
            ...
            for j in range(len(time_arr)):
                time_arr2[j%4].append(time_arr[j])
            print('time[{},{}] = [{}, {}, {}, {}]'.format(i,N-1, mean(time_arr2[0]), mean(time_arr2[1]), mean(time_arr2[2]), mean(time_arr2[3])))
